...
class CustomPainterWidget extends CustomPainter {
  final List<WaveLayer>
  waves; // List of wave layers to be drawn
  final List<BubbleWidget>
  bubbles; // List of bubble widgets to be drawn
  final double
  liquidLevel; // The current liquid level inside the bottle (0.0 to 1.0)
  final Color
  borderColor; // Border color for the bottle
  final Color capColor; // Cap color for the bottle
  ...
  void paint(Canvas canvas, Size size) {
    ...
    }
    {
      ...
      canvas.saveLayer(
        rect,
        paint,
      ); // Save the layer for better blending
      paintBottleMask(
        canvas,
        size,
        paint,
    ...
    }
    {
      ...
      paintWaves(
        ...
        size,
        paint,
      ); // Draw the waves inside the bottle
    }
    {
      ...
      paintBubbles(
        ...
        size,
        paint,
      ); // Draw the bubbles inside the bottle
    }
    {
    ...
    canvas
        .restore(); // Restore the canvas state after the mask
    {
  ...
  void paintWaves(
    ...
    for (var wave in waves) {
      ...
      final scaleX =
          ...
          wave.svgData
              .getBounds()
              .width; // Scale the wave width
      final scaleY =
          desiredH /
          wave.svgData
              .getBounds()
              .height; // Scale the wave height
  ...
  void paintBubbles(
    ...
    for (var bubble in bubbles) {
      paint.color =
          bubble.color; // Set the bubble color
      final offset = Offset(
        bubble.x * size.width,
  ...
  void paintGlossyOverlay(
    ...
    paint.color = Colors.white;
    paint.shader =
        gradient; // Apply the gradient for glossy effect
    canvas.drawRect(
  ...
  @override
  bool shouldRepaint(
    CustomPainterWidget oldDelegate,
  ) => true; // Always repaint
}
